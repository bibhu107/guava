...
class CompactHashMap<K extends @Nullable Object, V extends @Nullable Object>
  ...
  private int resizeTable(int mask, int newCapacity, int targetHash, int targetEntryIndex) {
    ...
    // Loop over current hashtable. The entries do not move, so the `keys` and `values` arrays do
    // not need to change. But because the "short hash" now has a different number of bits, we must
    // rewrite each element of `entries` so that its contribution to the full hashcode reflects the
    // change, and so that its `next` link corresponds to the new linked list of entries with the
    // new short hash. Rather than adding entries to the end of each of these new linked lists, we
    // insert them at the start, so we don't have to keep track of where the ends are.
